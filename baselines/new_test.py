...
class RedGymEnv(Env):
    ...
    def step(self, action):
        ...
        newx, newy, newmap_n = self.get_position()
        self.run_action_on_emulator(action)
        #if the next action is down and your on eithe rof those two tiles,
        x, y, map_n = self.get_position()
        if x != newx or y != newy:
            self.last_x_pos = newx
            self.last_y_pos = newy
            self.map_n = newmap_n
        print(f' the dierctional reward is {self.calculate_directional_reward()}')
    ...
    def calculate_directional_reward(self):
        
        target_x = 5
        target_y = 5
        new_x, new_y, map_n = self.get_position()
        if map_n == 59:
            # Calculate the initial distance to the target
            #initial cave (14,35, 59)
        # initial_distance = abs(target_x - current_x) + abs(target_y - current_y)
            initial_distance = abs(target_x - self.last_x_pos) + abs(target_y - self.last_y_pos)
            # Recalculate the distance after moving
           
            new_x, new_y, map_n = self.get_position()  # assuming get_position() returns updated position
            new_distance = abs(target_x - new_x) + abs(target_y - new_y)
            print(f'the new distance is {new_distance} and the initial distance is {initial_distance}')
    ...
    def get_all_events_reward(self):
        ...
        # event_flags_start = flags[0]
        # event_flags_end = flags[12]
        event_flags_start = MT_MOON_FLAG_START
        event_flags_end = MT_MOON_FLAG_END
  
        museum_ticket = (MUSEUM_TICKET_ADDRESS, 0)
        base_event_flags = 13
        x, y, map_n = self.get_position()
        # if self.step_count % 50 == 0:
        #     print(f"Map value:{map_n} menory at start {event_flags_start} {self.read_m(event_flags_start)} {self.bit_count(self.read_m(event_flags_start))}")
        #     print(f"Map value: {map_n} memory at end {event_flags_end} {self.read_m(event_flags_end)}")
        #return 0
        print(f'the {[self.bit_count(self.read_m(i)) for i in range(event_flags_start, event_flags_end)]}')
...
#         if self.step_count % 50 == 0:
#             for i in range(event_flags_start, event_flags_end):
#                     print(f"Memory value at {i}: {self.read_m(i)}")
...
# # 
    ...
    def update_max_event_rew(self):
        ...
        print(f'the curr reward is {cur_rew}')
